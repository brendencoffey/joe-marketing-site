---
title: Sips Coffee Grows Revenue with joe Point of Sale
date: 2022-08-10T00:00:00.000Z
author: Nick Martin
category: joe-pos
excerpt: >-
  How this Wyoming drive-thru grew revenue 17% by offering a hyper-convenient
  ordering experience through joe.
featured: false
featured_image: /images/blog/testimonials/sips-coffee-revenue.png
---

Sips Coffee, a drive-thru coffee shop in Wyoming, was looking for ways to grow revenue while providing a more convenient experience for their customers.

## The Challenge

As a drive-thru focused operation, Sips Coffee needed:
- Faster order processing
- A way to reduce wait times
- Tools to build customer loyalty

## The Solution

joe Point of Sale provided the perfect solution with:
- Built-in mobile ordering for skip-the-line convenience
- Automated loyalty rewards
- Seamless drive-thru workflow integration

## The Results

Since implementing joe, Sips Coffee has seen:
- **17% revenue growth**
- Reduced wait times
- Increased customer loyalty

The hyper-convenient ordering experience through joe has transformed how Sips Coffee serves their community, turning first-time visitors into loyal regulars.

— Owner, Sips Coffee
