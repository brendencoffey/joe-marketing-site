// ============================================
// DEALS MODULE - Full Implementation with Email
// ============================================
...
const Deals = {
    ...
    buildDetailHTML(deal, contacts, activities, tasks, stages) {
        ...
        const contactEmail = primaryContact?.email || deal.companies?.email || '';
        ...
        return `<div class="deal-detail">
            <div class="deal-header">
                <div class="deal-header-left">
                    ...
                    <div class="deal-title-section">
                        ...
                        <div class="deal-company">${deal.companies ? `<a href="#companies/${deal.companies.id}">${UI.escapeHtml(deal.companies.name)}</a> <span class="text-muted">${deal.companies.city || ''}, ${deal.companies.state || ''}</span>` : '<span class="text-muted">No company linked</span>'}</div>
                    </div>
                </div>
            ...
            <div class="stage-progress">
                ${stages.map((s, i) => {
                    ...
                    return `<div class="stage-step ${curr ? 'current' : ''} ${past ? 'completed' : ''}" style="--stage-color:${s.color}" onclick="Deals.changeStage('${s.id}')">
            ...
            <div class="deal-tabs">
                ...
                ${isCustomer ? '<button class="deal-tab" data-tab="onboarding">Onboarding</button><button class="deal-tab" data-tab="success">Success</button>' : '<button class="deal-tab" data-tab="sales">Sales</button>'}
            </div>
            ...
            <div class="deal-content">
                ...
                <div class="deal-tab-content" id="tab-activity">${this.renderActivity(deal, activities, contactEmail)}</div>
                <div class="deal-tab-content" id="tab-tasks">${this.renderTasks(tasks)}</div>
                <div class="deal-tab-content" id="tab-contacts">${this.renderContacts(deal, contacts)}</div>
                ${isCustomer ? `<div class="deal-tab-content" id="tab-onboarding">${this.renderOnboarding(deal)}</div>
    ...
    renderOverview(deal, contact, activities, tasks) {
        const contactEmail = contact?.email || deal.companies?.email || '';
        ...
        return `<div class="overview-grid">
            <div class="overview-main">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="quick-actions">
                            <button class="btn btn-primary" onclick="EmailActivity.showEmailComposer({dealId:'${deal.id}',companyId:'${deal.company_id||''}',toEmail:'${contactEmail}'})"><i data-lucide="mail"></i> Send Email</button>
                            <button class="btn btn-secondary" onclick="EmailActivity.showLogActivityModal({dealId:'${deal.id}',companyId:'${deal.company_id||''}',type:'call'})"><i data-lucide="phone"></i> Log Call</button>
                            <button class="btn btn-secondary" onclick="EmailActivity.showLogActivityModal({dealId:'${deal.id}',companyId:'${deal.company_id||''}',type:'note'})"><i data-lucide="file-text"></i> Add Note</button>
                            <button class="btn btn-secondary" onclick="Deals.createTask()"><i data-lucide="check-square"></i> Create Task</button>
                            <button class="btn btn-secondary" onclick="EmailActivity.showLogActivityModal({dealId:'${deal.id}',companyId:'${deal.company_id||''}',type:'meeting'})"><i data-lucide="calendar"></i> Log Meeting</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">${activities.length ? `<div class="activity-list">${activities.map(a => this.activityItem(a)).join('')}</div>` : '<p class="text-muted">No activity yet</p>'}</div>
                </div>
                
                <div class="card">
                    <div class="card-header"><h3>Pending Tasks</h3></div>
                    <div class="card-body">${tasks.length ? `<div class="task-list">${tasks.map(t => this.taskItem(t)).join('')}</div>` : '<p class="text-muted">No pending tasks</p>'}</div>
                </div>
            </div>
            ...
            <div class="overview-sidebar">
                ...
                <div class="card">
                    ...
                    <div class="card-body">${contact ? `<div class="contact-card">
                    ...
                    ${contact.email ? `<button class="btn btn-secondary btn-sm" style="margin-top:12px;width:100%;" onclick="EmailActivity.showEmailComposer({dealId:'${deal.id}',contactId:'${contact.id}',toEmail:'${contact.email}'})">
                        <i data-lucide="mail"></i> Email ${contact.first_name}
                    </button>` : ''}` : '<p class="text-muted">No contacts linked</p>'}</div>
                </div>
    ...
    renderActivity(deal, activities, contactEmail) {
        return `<div class="activity-header">
            <div class="activity-filters">
                <button class="activity-filter-btn active" onclick="Deals.filterActivities('')">All</button>
                <button class="activity-filter-btn" onclick="Deals.filterActivities('email')">Emails</button>
                <button class="activity-filter-btn" onclick="Deals.filterActivities('call')">Calls</button>
                <button class="activity-filter-btn" onclick="Deals.filterActivities('meeting')">Meetings</button>
                <button class="activity-filter-btn" onclick="Deals.filterActivities('note')">Notes</button>
            </div>
            <button class="btn btn-primary" onclick="EmailActivity.showEmailComposer({dealId:'${deal.id}',companyId:'${deal.company_id||''}',toEmail:'${contactEmail}'})">
                <i data-lucide="mail"></i> Send Email
            </button>
        </div>
        <div class="activity-timeline" id="activity-timeline">
            ${activities.length ? activities.map(a => this.activityItem(a, true)).join('') : '<div class="empty-state"><p>No activity recorded</p></div>'}
        </div>`;
    },
    
    activityItem(a, detailed = false) {
        const icons = { call: 'phone', email: 'mail', meeting: 'calendar', note: 'file-text', task: 'check-square', stage_change: 'git-branch' };
        const colors = { call: '#10b981', email: '#3b82f6', meeting: '#8b5cf6', note: '#6b7280', task: '#f59e0b' };
        const icon = icons[a.activity_type?.split('_')[0]] || 'activity';
        const color = colors[a.activity_type?.split('_')[0]] || '#6b7280';
        ...
        return `<div class="activity-item" data-type="${a.activity_type}">
            ...
            <div class="activity-content">
                ...
                ${detailed && (a.body || a.notes) ? `<div class="activity-body">${UI.escapeHtml(a.body || a.notes).substring(0, 300)}${(a.body || a.notes)?.length > 300 ? '...' : ''}</div>` : ''}
                ${a.outcome ? `<span class="activity-outcome">${a.outcome.replace(/_/g, ' ')}</span>` : ''}
            </div>
        </div>`;
    },
            ...
            return `<div class="contact-card-large">
                ...
                <div class="contact-details">
                    ...
                    <div class="contact-actions">
                        ${c.email ? `<button class="btn btn-ghost btn-sm" onclick="EmailActivity.showEmailComposer({dealId:'${deal.id}',contactId:'${c.id}',toEmail:'${c.email}'})"><i data-lucide="mail"></i></button>` : ''}
                        ${c.phone ? `<a href="tel:${c.phone}" class="btn btn-ghost btn-sm"><i data-lucide="phone"></i></a>` : ''}
                    </div>
                </div>
            </div>`;
        }).join('')}</div>` : '<div class="empty-state"><p>No contacts linked to this deal</p></div>'}`;
    },
    ...
    createTask() {
        UI.modal({
            ...
            onConfirm: async () => {
                ...
                if (!d.title) { UI.toast('Task title is required', 'error'); return; }
    ...
    filterActivities(type) {
        ...
        document.querySelectorAll('.activity-filter-btn').forEach(btn => {
            ...
            btn.classList.toggle('active', (!type && btnType === 'all') || btnType.includes(type));
        });
