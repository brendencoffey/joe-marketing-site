...
const Pipeline = {
  ...
  async loadData() {
    ...
    // Load deals with company info for allowed pipelines only
    const { data: deals } = await db
      .from('deals')
      .select('*, companies(id, name, city, state)')
      .in('pipeline_id', this.ALLOWED_PIPELINES)
      .order('created_at', { ascending: false });
    if (deals) Store.data.deals = deals;
  ...
  renderBoard() {
    ...
    if (ownerFilter) {
      deals = deals.filter(d => d.assigned_to === ownerFilter || d.owner_email === ownerFilter || d.owner_id === ownerFilter);
    }
        ...
        <div class="pipeline-column" data-stage-id="${stage.id}">
          <div class="pipeline-column-header">
            <div class="pipeline-column-title">
              <span class="stage-dot" style="background: ${stage.color || '#666'}"></span>
              ${stage.name}
  ...
  renderDealCard(deal) {
    ...
    const owner = Store.data.teamMembers?.find(t => t.id === deal.owner_id || t.email === deal.assigned_to);
    ...
    return `
      <div class="deal-card" 
           ...
           onclick="Pipeline.openDeal('${deal.id}')">
        <div class="deal-card-header">
          <div class="deal-card-title">${UI.escapeHtml(deal.name || 'Unnamed Deal')}</div>
          ${deal.value ? `<span class="deal-card-value">$${deal.value.toLocaleString()}</span>` : ''}
        </div>
        <div class="deal-card-company">${company ? UI.escapeHtml(company.name) : '-'}</div>
        <div class="deal-card-footer">
          <div class="deal-card-owner">
            ${owner?.avatar_url 
              ? `<img src="${owner.avatar_url}" alt="${owner.name}">`
              : `<span class="owner-initials">${owner?.name?.charAt(0) || '?'}</span>`
            }
            <span>${owner?.name?.split(' ')[0] || '-'}</span>
          </div>
          <span class="deal-card-days">${daysInStage}d</span>
        </div>
      </div>
    `;
  },
  ...
  async onDrop(event, stageId) {
    ...
    if (!error) {
      ...
      if (deal) {
        ...
        deal.stage_changed_at = new Date().toISOString();
      }
      this.renderBoard();
